import { Environment } from "./eva-environment.js";
import { Transformer } from "./eva-transformer.js";
	...
	eval(exp, env = this.global) {
		...
		if (exp[0] === "set") {
			...
			if(ref[0] === "prop"){
				...
				if(!instanceEnv.define) debugger;
		...
		//syntic sugar for (var square (lambda (x) (* x x))
		if(exp[0] === "def"){
		...
		if (exp[0] === "+=") {
			const setExp = this._transformer.transformAddAssignToSet(exp);
			return this.eval(setExp, env);
		}
		if (exp[0] === "-=") {
			const setExp = this._transformer.transformSubAssignToSet(exp);
			return this.eval(setExp, env);
		}
		if(exp[0] === "lambda"){
		...
		if(exp[0] === "import"){
			...
			const src = [];  //figure out how to import 
			src.unshift("begin");
